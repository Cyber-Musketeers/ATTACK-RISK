...
from stix2.properties import (
    BooleanProperty, IDProperty, ListProperty, ReferenceProperty,
    StringProperty, TimestampProperty, EnumProperty
)


@CustomObject('attack-flow', [
    ...
    ('start_refs', ListProperty(ReferenceProperty(valid_types=['attack-action', 'attack-condition']), required=True)),
])
class AttackFlow(_STIXBase21):
    pass

@CustomObject('attack-action', [
    ...
    ('effect_refs', ListProperty(ReferenceProperty(valid_types=['attack-action', 'attack-operator', 'attack-condition']))),
])
class AttackAction(_STIXBase21):
    pass

@CustomObject('attack-asset', [
    ('type', StringProperty(required=True, fixed='attack-asset')),
    ('spec_version', StringProperty(required=True, fixed='2.1')),
    ('name', StringProperty(required=True)),
    ('description', StringProperty()),
    ('object_ref', ReferenceProperty(invalid_types=[])),  # Allow any STIX object reference
])
class AttackAsset(_STIXBase21):
    pass

@CustomObject('attack-condition', [
    ...
    ('on_true_refs', ListProperty(ReferenceProperty(valid_types=['attack-action', 'attack-operator', 'attack-condition']))),
    ('on_false_refs', ListProperty(ReferenceProperty(valid_types=['attack-action', 'attack-operator', 'attack-condition']))),
])
class AttackCondition(_STIXBase21):
    pass

@CustomObject('attack-operator', [
